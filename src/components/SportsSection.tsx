      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center space-y-4 mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-pill text-xs sm:text-sm font-bold uppercase tracking-wider text-[#0066FF] shadow-2xs">
            <Sparkles className="w-4 h-4 text-[#0066FF]" /> Multi-Sport Coverage
          </div>
          <h2 className="text-3xl sm:text-5xl md:text-6xl font-black text-slate-950 tracking-tight leading-[1.12]">
          ...
          {sportsList.map((sport) => {
            ...
            return (
              <button
                ...
                className={`px-4 sm:px-5 py-2.5 sm:py-3 rounded-full text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-200 shrink-0 flex items-center gap-2 cursor-pointer ${
                  isSelected
                    ? 'bg-slate-950 text-white shadow-md ring-2 ring-slate-900 scale-102'
                    : 'glass-pill text-slate-700 hover:text-slate-950 hover:bg-white shadow-2xs'
                }`}
              >
                <span className={`w-2 h-2 rounded-full ${isSelected ? 'bg-[#0066FF]' : 'bg-slate-300'}`} />
        ...
        <div className="squash-card-navy rounded-3xl p-6 sm:p-12 mb-8 sm:mb-12 shadow-2xl border border-[#1d2d5b] transition-all duration-300">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-center">
            
            {/* Left Detail Info */}
            <div className="lg:col-span-7 space-y-4 text-left">
              <div className="flex flex-wrap items-center gap-2.5">
                <span className="px-3 py-1 rounded-full bg-[#0066FF]/20 text-[#0066FF] text-xs font-mono font-bold border border-blue-500/30">
                  {activeSportData.type}
                </span>
                <span className="px-3 py-1 rounded-full bg-[#061538] text-[#8e9cbc] text-xs font-mono font-semibold border border-[#1d2d5b]">
              ...
              <p className="text-[#8e9cbc] text-sm sm:text-lg leading-relaxed font-normal">
                {activeSportData.description}
              </p>

              <div className="pt-1 sm:pt-2 max-w-xs">
                <div className="bg-[#061538] p-3.5 sm:p-4 rounded-2xl border border-[#1d2d5b]">
                  <div className="text-xs uppercase font-mono font-bold text-[#8e9cbc]">Match Format</div>
                  <div className="text-sm sm:text-base font-extrabold text-white mt-0.5">{activeSportData.format}</div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 flex justify-center">
              <div className="w-full max-w-sm p-5 sm:p-7 rounded-2xl bg-[#000d2a] text-white shadow-2xl space-y-4 border border-[#404f9a]/60">
                <div className="flex items-center justify-between border-b border-[#1d2d5b] pb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-[#0066FF] flex items-center justify-center font-black text-white text-sm shadow-md">
                      CU
                    </div>
                    <div>
                      <div className="text-sm font-extrabold text-white">{activeSportData.name} Match</div>
                      <div className="text-xs font-mono text-[#8e9cbc]">CircleUp Engine</div>
                    </div>
                  </div>
                  <span className="text-xs font-mono font-bold px-3 py-1 rounded-full bg-[#39d078]/15 text-[#39d078] border border-[#39d078]/30 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-[#39d078] animate-pulse" /> Ready
                  </span>
                </div>

                <div className="space-y-2 text-xs sm:text-sm text-[#8e9cbc]">
                  <div className="flex items-center justify-between py-1.5 border-b border-[#1d2d5b]/60">
                  ...
                  <div className="flex items-center justify-between py-1.5">
                    ...
                    <span className="font-semibold text-[#39d078] flex items-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4 text-[#39d078]" /> Direct Sync
                    </span>
                  </div>
                </div>

                <div className="pt-1.5">
                  <div className="text-xs font-mono text-[#8e9cbc] text-center font-bold tracking-wider">
          ...
          {sportsList.map((sport) => {
            ...
            return (
              <div
                key={sport.id}
                onClick={() => setSelectedSport(sport.id)}
                className={`p-4 sm:p-6 rounded-2xl cursor-pointer transition-all duration-200 text-left ${
                  isSelected
                    ? 'squash-card-navy ring-2 ring-[#39d078] shadow-xl scale-[1.02]'
                    : 'bg-white hover:bg-slate-50 border border-slate-200/90 shadow-sm hover:border-[#0066FF]/50'
                }`}
              >
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <span className={`text-[11px] sm:text-xs font-mono font-bold uppercase tracking-wider ${isSelected ? 'text-[#39d078]' : 'text-slate-400'}`}>
                    {sport.type}
                  </span>
                  {isSelected && (
                    <span className="w-2.5 h-2.5 rounded-full bg-[#39d078] animate-pulse" />
                  )}
                </div>
                <h4 className={`text-lg sm:text-2xl font-black tracking-tight ${isSelected ? 'text-white' : 'text-slate-950'}`}>
                  {sport.name}
                </h4>
                <p className={`text-xs sm:text-sm mt-1 sm:mt-1.5 line-clamp-2 ${isSelected ? 'text-[#8e9cbc]' : 'text-slate-500 font-medium'}`}>
                  {sport.format}
                </p>
              </div>
            );
